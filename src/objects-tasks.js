  ...
  return objects.reduce((acc, obj) => {
    Object.entries(obj).forEach(([key, value]) => {
      if (acc[key] !== undefined) {
        if (typeof acc[key] === 'number' && typeof value === 'number') {
          acc[key] += value;
        } else {
...
function compareObjects(obj1, obj2) {
  ...
  if (typeof keys1 !== typeof keys2 && keys1.length !== keys2.length) {
    return false;
  }
  const result = keys1.every((key) => obj1[key] === obj2[key]);
  return result;
}
...
function isEmptyObject(obj) {
  ...
  return keys1.every((key) => obj[key] === 0);
}
